<template>
  <div class="project-config-page">
    <div class="page-header">
      <button class="back-btn" @click="router.back()">
        <ArrowLeft :size="20" />
        返回
      </button>
      <h1>Builder 配置</h1>
    </div>
    <div class="page-content">
      <div class="config-placeholder">
        <Package :size="48" class="config-icon" />
        <p>Builder 配置页面</p>
        <p class="config-hint">配置文件：.ldesign/builder.config.ts</p>
        <div class="config-info">
          <p class="info-title">Builder 配置说明</p>
          <ul class="info-list">
            <li>支持零配置，自动检测项目类型和入口文件</li>
            <li>支持多格式输出：ESM、CJS、UMD</li>
            <li>支持多引擎：esbuild、swc、rollup、rolldown</li>
            <li>支持 13 种前端框架自动检测</li>
            <li>支持增量构建和智能缓存</li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { useRoute, useRouter } from 'vue-router'
import { ArrowLeft, Package } from 'lucide-vue-next'

const route = useRoute()
const router = useRouter()
</script>

<style scoped>
.project-config-page {
  padding: var(--content-padding);
  background: var(--content-bg);
  color: var(--color-text-primary);
  min-height: 100%;
}

.page-header {
  display: flex;
  align-items: center;
  gap: var(--size-spacing-md);
  margin-bottom: var(--size-spacing-xl);
}

.back-btn {
  display: flex;
  align-items: center;
  gap: var(--size-spacing-xs);
  padding: var(--size-spacing-sm) var(--size-spacing-md);
  background: var(--color-bg-component);
  color: var(--color-text-primary);
  border: 1px solid var(--color-border-light);
  border-radius: var(--size-radius-md);
  cursor: pointer;
  font-size: var(--font-size-base);
  transition: all 0.2s ease;
}

.back-btn:hover {
  background: var(--color-bg-component-hover);
  border-color: var(--theme-color-primary);
}

.page-header h1 {
  font-size: var(--font-size-2xl);
  font-weight: var(--size-font-weight-semibold);
  margin: 0;
  color: var(--color-text-primary);
}

.page-content {
  max-width: 1200px;
}

.config-placeholder {
  text-align: center;
  padding: var(--size-spacing-3xl);
  background: var(--color-bg-container);
  border: 1px solid var(--color-border-light);
  border-radius: var(--size-radius-lg);
}

.config-icon {
  color: var(--theme-color-primary);
  margin-bottom: var(--size-spacing-lg);
}

.config-placeholder p {
  font-size: var(--font-size-lg);
  color: var(--color-text-primary);
  margin: 0 0 var(--size-spacing-sm) 0;
}

.config-hint {
  font-size: var(--font-size-sm);
  color: var(--color-text-secondary);
  margin-bottom: var(--size-spacing-xl);
}

.config-info {
  margin-top: var(--size-spacing-xl);
  padding-top: var(--size-spacing-xl);
  border-top: 1px solid var(--color-border-light);
  text-align: left;
  max-width: 600px;
  margin-left: auto;
  margin-right: auto;
}

.info-title {
  font-size: var(--font-size-base);
  font-weight: var(--size-font-weight-semibold);
  color: var(--color-text-primary);
  margin-bottom: var(--size-spacing-md);
}

.info-list {
  list-style: none;
  padding: 0;
  margin: 0;
}

.info-list li {
  padding: var(--size-spacing-sm) 0;
  padding-left: var(--size-spacing-lg);
  position: relative;
  color: var(--color-text-secondary);
  font-size: var(--font-size-sm);
  line-height: 1.6;
}

.info-list li::before {
  content: '✓';
  position: absolute;
  left: 0;
  color: var(--color-success-default);
  font-weight: var(--size-font-weight-bold);
}
</style>
